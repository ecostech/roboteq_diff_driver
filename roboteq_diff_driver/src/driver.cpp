...
void MainNode::cmdvel_callback( const geometry_msgs::Twist& msg)
{
  twist_msg = msg;
  run_loop = true;
//   // wheel speed (m/s)
...
//ROS_DEBUG_STREAM( "odom x: " << odom_x << " y: " << odom_y << " yaw: " << odom_yaw );
  ...
  float vyaw = NORMALIZE(odom_yaw - odom_last_yaw) / dt;
#ifdef _ODOM_DEBUG
...
int MainNode::run()
{
  ...
  mstimer = starttime;
  lstimer = starttime;
  cmdvel_timer = starttime;
  odom_timer = starttime;
